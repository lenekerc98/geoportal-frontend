...
export default function Geoportal() {
  ...
  const [showPredios, setShowPredios] = useState(() => {
    if (location.state?.selectPredioId) return true;
    try {
      const savedEmp = localStorage.getItem('catastro_active_empresa');
      if (savedEmp) {
        const parsed = JSON.parse(savedEmp);
        if (parsed?.parametros?.capa_predios_activa !== undefined) {
  ...
  useEffect(() => {
    if (activeEmpresa?.parametros?.capa_predios_activa !== undefined) {
      setShowPredios(Boolean(activeEmpresa.parametros.capa_predios_activa));
    }
  }, [activeEmpresa?.id, activeEmpresa?.parametros?.capa_predios_activa]);

  // Recargar datos si cambia la empresa o el proyecto activo
  useEffect(() => {
    if (showPredios && authToken) {
      fetchMapData();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.tipo === 'raster') {
            ...
            if (data.capa === 'predio') { setPrediosData(null); if (showPredios) fetchMapData(); }
